...
import { Mail, Plus, Edit, Copy, Check, Eye, Send, Filter, Search, Calendar, BarChart3, Users } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">Manage and optimize your email marketing campaigns</p>
        </div>
            ...
            <div key={campaign.id} className="p-6 hover:bg-gray-800/50 transition-colors">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-gray-400 text-sm mb-2">To: {campaign.persona}</p>
                  ...
                  {campaign.status === 'sent' && (
                    <div className="flex items-center gap-6 mt-3 text-sm">
                      ...
                      <span className="text-gray-400">Recipients: {campaign.recipients.toLocaleString()}</span>
                      <span className="text-gray-400">Open: {campaign.openRate}</span>
                      <span className="text-gray-400">Click: {campaign.clickRate}</span>
                      <span className="text-gray-400">Revenue: {campaign.revenue}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => copyToClipboard(campaign.subject, `subject-${campaign.id}`)}
                  ...
                  >
                    {copiedText === `subject-${campaign.id}` ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                  </button>
                  <button
          ...
          <div className="bg-[#1A1A1A] rounded-lg border border-gray-800 p-6 max-w-4xl w-full mx-4 max-h-[80vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2">
                <div className="bg-gray-800 rounded-lg p-4 mb-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      onClick={() => copyToClipboard(selectedCampaign.subject, 'modal-subject')}
                    ...
                    >
                      {copiedText === 'modal-subject' ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                ...
                <div className="bg-gray-800 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-semibold text-white">Email Content</h3>
                    <button
                      onClick={() => copyToClipboard(selectedCampaign.content, 'modal-content')}
                      className={`flex items-center gap-2 px-3 py-1 rounded text-sm transition-colors ${
                  ...
                  <div className="text-gray-300 whitespace-pre-line">{selectedCampaign.content}</div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="bg-gray-800 rounded-lg p-4">
                  <h3 className="font-semibold text-white mb-3">Campaign Details</h3>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-gray-400">Target Persona:</span>
                      <span className="text-white">{selectedCampaign.persona}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Status:</span>
                      <span className={`px-2 py-1 rounded text-xs text-white ${getStatusColor(selectedCampaign.status)}`}>
                    ...
                    {selectedCampaign.sentDate && (
                      <div className="flex justify-between">
                        ...
                        <span className="text-white">{new Date(selectedCampaign.sentDate).toLocaleDateString()}</span>
                      </div>
                    )}
                    <div className="flex justify-between">
                      <span className="text-gray-400">Recipients:</span>
                      <span className="text-white">{selectedCampaign.recipients.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span className="text-white">{selectedCampaign.openRate}</span>
                        </div>
                        <div className="w-full bg-gray-700 rounded-full h-2">
                          <div className="bg-green-600 h-2 rounded-full" style={{width: selectedCampaign.openRate}}></div>
                        </div>
                      </div>
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                          <span className="text-gray-400">Click Rate</span>
                          <span className="text-white">{selectedCampaign.clickRate}</span>
                        </div>
                        <div className="w-full bg-gray-700 rounded-full h-2">
                          <div className="bg-blue-600 h-2 rounded-full" style={{width: selectedCampaign.clickRate}}></div>
                        </div>
                      </div>
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                          <span className="text-gray-400">Conversion</span>
                          <span className="text-white">{selectedCampaign.conversionRate}</span>
                        </div>
                        <div className="w-full bg-gray-700 rounded-full h-2">
                          <div className="bg-purple-600 h-2 rounded-full" style={{width: selectedCampaign.conversionRate}}></div>
                        </div>
                      </div>
                      <div className="pt-2 border-t border-gray-700">
                        <span className="text-gray-400 text-sm">Revenue Generated:</span>
                        <span className="text-green-500 font-semibold ml-2">{selectedCampaign.revenue}</span>
                      </div>
                    </div>
                  </div>
                )}
