...
import { TrendingUp, TrendingDown, Target, Award, Users, Mail, Share2, BarChart3, Clock, Calendar, Filter, RefreshCw } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Monitor and analyze your marketing performance</p>
        </div>
          ...
          <Card key={index}>
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div className={`p-2 rounded-lg ${getPerformanceBg(metric.score)}`}>
              ...
              <div className="space-y-2">
                <div className={`text-3xl font-bold ${getPerformanceColor(metric.score)}`}>
                  {metric.score}
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  {timelineData.map((week, index) => (
                    <div key={index} className="space-y-2">
                      <div className="flex justify-between text-sm font-medium">
                        <span>{week.date}</span>
                        <span className="text-gray-600">Overall: {week.overallPerformance}%</span>
                      </div>
                      <div className="space-y-1">
                        ...
                        <Progress value={week.emailPerformance} className="h-1" />
                        <div className="flex justify-between text-xs">
                          <span>Social</span>
                          <span>{week.socialPerformance}%</span>
                        </div>
                        <Progress value={week.socialPerformance} className="h-1" />
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Your best performing pieces this month</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-center gap-4 p-3 border rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-600">Email Campaign</div>
                    </div>
                  ...
                  <div className="flex items-center gap-4 p-3 border rounded-lg">
                    ...
                    <Badge className="bg-blue-100 text-blue-800">89% Score</Badge>
                  </div>
                  ...
                  <div className="flex items-center gap-4 p-3 border rounded-lg">
                    ...
                    <div className="flex-1">
                      <div className="font-medium text-sm">Customer Success Story</div>
                      <div className="text-xs text-gray-600">Blog Post</div>
                    </div>
                    <Badge className="bg-purple-100 text-purple-800">87% Score</Badge>
                  </div>
                </div>
              </CardContent>
            </Card>
                      ...
                      <tr key={index} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                            <Progress value={campaign.performance} className="w-16 h-2" />
                            <span className={`text-sm font-medium ${getPerformanceColor(campaign.performance)}`}>
                              {campaign.performance}%
                            </span>
                          </div>
                        </td>
                        <td className="py-3 px-4 font-medium text-green-600">{campaign.revenue}</td>
                        <td className="py-3 px-4 text-gray-600">{campaign.conversions}</td>
                        <td className="py-3 px-4 font-medium">{campaign.roi}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <div key={index} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-600 mt-1">
                        Your performance: {benchmark.yourPerformance} | Industry avg: {benchmark.industryAverage}
                      </div>
                    </div>
                    <div className="text-right">
                      <Badge variant={benchmark.performance === 'above' ? 'default' : 'destructive'}>
                        {benchmark.performance === 'above' ? 'Above Average' : 'Below Average'}
                      </Badge>
                      <div className={`text-sm font-medium mt-1 ${
                        benchmark.performance === 'above' ? 'text-green-600' : 'text-red-600'
                      }`}>
