...
import { BarChart3, TrendingUp, TrendingDown, Users, Mail, Share2, MousePointer, Eye, DollarSign, Calendar, Filter, Download } from 'lucide-react';
                  ...
                  <tr key={index} className="border-b hover:bg-gray-50">
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-3">
                        {channel.channel === "Email Marketing" && <Mail className="h-4 w-4 text-blue-500" />}
                        {channel.channel === "Social Media" && <Share2 className="h-4 w-4 text-purple-500" />}
                        {channel.channel === "Google Ads" && <BarChart3 className="h-4 w-4 text-green-500" />}
                        <span className="font-medium">{channel.channel}</span>
                      </div>
                    </td>
                    <td className="py-3 px-4 text-gray-600">{channel.impressions}</td>
                    <td className="py-3 px-4 text-gray-600">{channel.clicks}</td>
                    <td className="py-3 px-4">
                      <Badge variant="secondary">{channel.ctr}</Badge>
                    </td>
                    <td className="py-3 px-4 text-gray-600">{channel.conversions}</td>
                    <td className="py-3 px-4 font-medium text-green-600">{channel.revenue}</td>
                    <td className="py-3 px-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Your best campaigns by ROI and revenue</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {topCampaigns.map((campaign, index) => (
                <div key={index} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-600 mt-1">{campaign.type} • {campaign.impressions} impressions</div>
                  ...
                  <div className="text-right">
                    <div className="font-bold text-green-600">{campaign.revenue}</div>
                ...
                <div key={index} className="p-4 border rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div className="col-span-2">
                      ...
                      <div className="font-medium text-green-600">{segment.avgOrderValue}</div>
                    </div>
                  </div>
                </div>
              ))}
