...
import { Target, Users, Mail, Share2, Image, Wand2, Save, Play, Eye, ChevronDown, Calendar, DollarSign } from "lucide-react";
...
const CampaignBuilder = () => {
  ...
  const steps = [
    ...
    { id: 2, title: "Audience & Targeting", icon: <Users className="w-5 h-5" /> },
  ...
  const objectives = [
    ...
    { id: "engagement", label: "Engagement", description: "Drive interactions and conversations" },
    { id: "leads", label: "Lead Generation", description: "Capture potential customer information" },
    { id: "sales", label: "Sales", description: "Drive direct purchases and conversions" },
  ...
  const channels = [
    ...
    { id: "display", label: "Display Ads", icon: <Image className="w-5 h-5" /> },
  ...
  const renderStep1 = () => (
    <div className="space-y-6">
      ...
      <div>
        <label className="block text-white text-sm font-medium mb-2">Campaign Objective</label>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {objectives.map((objective) => (
            ...
            >
              ...
              <p className="text-gray-400 text-sm mt-1">{objective.description}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  const renderStep2 = () => (
    <div className="space-y-6">
      <div>
        <label className="block text-white text-sm font-medium mb-2">Target Persona</label>
      ...
      <div>
        <label className="block text-white text-sm font-medium mb-2">Marketing Channels</label>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
  ...
  const renderStep3 = () => (
    <div className="space-y-6">
      <div>
        <label className="block text-white text-sm font-medium mb-2">Content Types</label>
      ...
      <div>
        <label className="block text-white text-sm font-medium mb-2">Tone of Voice</label>
      ...
      <div>
        <label className="block text-white text-sm font-medium mb-2">Keywords & Topics</label>
        <textarea
  ...
  const renderStep4 = () => (
    <div className="space-y-6">
      <div className="bg-gray-800 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <span className="text-white ml-2">{campaignData.name || "Not set"}</span>
          </div>
          <div>
            <span className="text-gray-400">Objective:</span>
            <span className="text-white ml-2">{objectives.find(o => o.id === campaignData.objective)?.label || "Not set"}</span>
          </div>
          <div>
            <span className="text-gray-400">Persona:</span>
            <span className="text-white ml-2">{personas.find(p => p.id === campaignData.persona)?.label || "Not set"}</span>
          </div>
          <div>
            <span className="text-gray-400">Budget:</span>
            <span className="text-white ml-2">${campaignData.budget || "Not set"}</span>
          </div>
          <div>
            <span className="text-gray-400">Duration:</span>
            <span className="text-white ml-2">{campaignData.startDate && campaignData.endDate ? `${campaignData.startDate} to ${campaignData.endDate}` : "Not set"}</span>
          </div>
          <div>
            <span className="text-gray-400">Channels:</span>
            <span className="text-white ml-2">{campaignData.channels.length > 0 ? campaignData.channels.join(", ") : "None selected"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-green-800/20 border border-green-700 rounded-lg p-4">
        ...
        <p className="text-gray-300 text-sm">Your campaign is configured and ready to generate content. Click "Generate Campaign" to create your marketing materials.</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">Create AI-powered marketing campaigns in minutes</p>
        </div>
            ...
            <div key={step.id} className="flex items-center">
              <div
                className={`flex items-center gap-3 cursor-pointer ${
                  currentStep === step.id ? 'text-blue-500' : currentStep > step.id ? 'text-green-500' : 'text-gray-500'
                }`}
                onClick={() => setCurrentStep(step.id)}
              >
                <div className={`p-2 rounded-full ${
                  currentStep === step.id ? 'bg-blue-500' : currentStep > step.id ? 'bg-green-500' : 'bg-gray-700'
                }`}>
                  {step.icon}
                </div>
              ...
              {index < steps.length - 1 && (
                <div className={`w-12 h-0.5 mx-4 ${currentStep > step.id ? 'bg-green-500' : 'bg-gray-700'}`} />
              )}
            </div>
          ))}
        </div>
